...
test("the compiled dashboard serves its public assets and garden API", async () => {
  ...
  try {
    ...
    assert.match(cssText, /data-shape="lace"/);
    for (const species of ["moonbell", "starpetal", "rainmint", "emberbloom", "duskfern", "cloverlight", "snowlace", "sunsigh", "tideglass", "velvethorn", "lanternmoss", "cloudpoppy"]) {
    ...
    assert.match(scriptText, /localizedWeather\(weather\.condition\)/);
    assert.match(scriptText, /localizedTime\(timeOfDay\)/);
